...
---
## This script is a modified version of what we submitted to cogsci. It makes sure that the transition matrices also include the transitions from and to the introduction moments. It also includes a sliding window analysis that replicates the increase in surprisal.  This is also what we used to validate findings from our minimal model of mathematical insights.
...
```{r}
# In case you run the code outside of the math environment: "/Users/shaditaba/Desktop/cogs_courses/Math_insight_Project/data/insight_math_UPDATED.csv"
insight_data = read_csv(file="/Users/shaditaba/Desktop/cogs_courses/Math_insight_Project/data/insight_math_UPDATED.csv")
```

```{r}
# In case you run the code outside of the math environment:
#"/Users/shaditaba/Desktop/cogs_courses/Math_insight_Project/data/mathExperts_allData.txt"
experts <- read.table(file = "/Users/shaditaba/Desktop/cogs_courses/Math_insight_Project/data/mathExperts_allData.txt", 
                      sep='\t', 
                      skip = 1)
...
# In case you run the code outside of the math environment:
#"/Users/shaditaba/Desktop/cogs_courses/Math_insight_Project/data/Experts-RecodingObjectsIntoClusters.csv"
...
newObjectLabels <- read.csv(file = "/Users/shaditaba/Desktop/cogs_courses/Math_insight_Project/data/Experts-RecodingObjectsIntoClusters.csv")
...
#pay attention that we filter for attention to get rid of the insight moment since they do not have an object. Intro moments are not marked in this dataset (experts_and_insights_clean).
...
#Marking the relation of shifts if attention to the moment of insight
...
# The following bit was added by Tyler to center the model on other times as opposed to on 0.
All_measures_and_events <- All_measures_and_events %>% 
  mutate(isOtherTimes = !(isBeforeInsight | isAfterInsight))

All_measures_and_events.ml2 <- glmer(event10 ~ 1 + isOtherTimes * isAfterInsight + startMinutes + 
                                       (1|subProb), 
                                     data = All_measures_and_events, 
                                     family = "binomial", 
                                     verbose=T)
...
All_measures_and_events %>%
  group_by(isBeforeInsight, isAfterInsight) %>%
  summarize(event10 = mean(event10))

#####the end of the bit added by Tyler
...
measures_dataframe %>% 
  ...
  #scale_x_discrete(labels = c ( 1 = "non-insight" ,  3 = "pre-insight" , 4 = "post-insight"))
...
###modify the look for dissertaion
measures_dataframe %>% 
  # filter(plot_x != "both") %>%
  ggplot(aes(x= plot_x, y = ent_MM, color = illumination)) + 
  geom_pointrange(aes(ymin = ent_MM - ent_SE, 
...
#probability of new inscriptions relative to the moment of insight
measures_dataframe %>% 
  # filter(plot_x != "both") %>%
  ggplot(aes(x= plot_x, y = new_objects_MM, color = illumination)) + 
  geom_pointrange(aes(ymin=new_objects_MM-new_objects_SE, 
                      ymax=new_objects_MM + new_objects_SE), size = 2) +
  
  ylab("P(new inscription)") +
  scale_x_continuous( label = c( "non-insight", "pre-insight", "     post-insight"), breaks = c(1,3,4), limits = c(0,4.5)) +
  #xlab("Relation to insight") +
  #scale_x_discrete(labels = c ( 1 = "non-insight" ,  3 = "pre-insight" , 4 = "post-insight"))
  scale_y_continuous (expand = c(0,0), limits= c(0.01, 0.09))+
  #xlab("Relation to insight") +
  #scale_x_discrete(labels = c ( 1 = "non-insight" ,  3 = "pre-insight" , 4 = "post-insight"))
  theme_classic(base_size = 15)+
  theme(axis.text = element_text(size = 30)) +
...
##Tyler's addition to replicate our analysis using a sliding window
Pull out relevant periods, and recenter the time variable so it's centered on the first insight:
```{r, fig.width = 10, fig.height = 6}
...
# sliding_window_df.insightPeriods %>%
#   ggplot(aes(x=start_first_c, y=surprisalM)) +
#   geom_line() +
#   geom_vline(xintercept=0) +
#   facet_wrap(~subProb, scales="free")
...
#ggsave("../pic/nice_plot.png", plot = actual_slide)
```
...
#### Deniz triangle session plots to describe our time series during a presentation

```{r, fig.height=4, fig.width=6}

## Plot for Deniz objects
Deniz_objects_triangle <- experts_and_insights_clean %>% 
  filter (Event != "insight") %>%
  subset(subName== "Deniz" & problemName == "Triangle")%>%
  ggplot() +
  geom_point( aes(x = start/1000, y = factor(obNew),
                  color= factor(obNew), size = 1)) +
  scale_x_continuous(expand = c(0.03,0), limits = c(0, 1000)) 

plot_3<-Deniz_objects_triangle + 
  #geom_vline(xintercept = c(513, 529, 742), color = "red", lty = "dashed", size = 1) +
  xlab("Time (seconds)") + 
  ylab("Inscription ID")+ 
  scale_y_discrete(breaks= seq(0,21,5)) + 
  # This code allows us to enforce the numbers we want to show up on the axis
  theme_classic()+
  theme(text = element_text(size = 25)) +    
  theme(axis.text = element_text(size = 25)) + # All font sizes
  theme(legend.position="non") # Removing the legend
plot_3


ggsave(plot = plot_3,"deniz9.pdf", path ="../minimal model/Figures/")
